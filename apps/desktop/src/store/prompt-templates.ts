import { LOCAL_KAZIKE_SEED } from './prompt-templates-kazike'

function migrateLegacyPromptSnippets(): void {
  // HERMES-LOCAL-PROMPT-MIGRATE
  if (typeof window === 'undefined') {
    return
  }
  try {
    const dest = "hermes.desktop.prompt-templates"
    for (const srcKey of ["hermes-desktop-prompt-snippets-v1", "hermes.desktop.prompt-snippets"]) {
      ...
      if (Array.isArray(items) && items.length > 0) {
        window.localStorage.setItem(dest, JSON.stringify(items))
      }
      window.localStorage.removeItem(srcKey)
    }
  } catch {
    // ignore corrupt legacy keys
  }
}
migrateLegacyPromptSnippets()
/**
...
import { getStarterFolderSeed } from './starter-prompt-seed'
...
export const BUILTIN_TEMPLATE_IDS = ["socraticQuestioning", "dualLayerExplanation", "reverseDeconstruction", "crossLongitudinalAnalysis", "factCheck", "expertConsultation", "firstPrinciples", "crossDomainTransfer", "dualSteelman", "minimalExperiment", "hiddenTalent", "lifeDesign"] as const
...
export function getBuiltInTemplates(): PromptTemplate[] {
  // HERMES-LOCAL-KAZIKE-SEED
  return LOCAL_KAZIKE_SEED.map(item => ({ ...item }))
}
...
export function resetToBuiltins(): void {
  // Local dogfood only — upstream PR keeps getBuiltInTemplates()
  setList(getStarterFolderSeed())
}
