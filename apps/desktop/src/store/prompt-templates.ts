...
export function getBuiltInTemplates(): PromptTemplate[] {
  // Upstream-shaped 3 starters (i18n). Local dogfood first-open seed is empty via getStarterFolderSeed().
  // Packs (卡兹克/姜) live only in ~/.hermes/prompt-packs + inbox — never as builtins.
  return ([
    'codeReview',
    'implementationPlan',
    'explainThis'
    ...
    parentId: null,
    label: translateNow(`composer.promptTemplates.builtins.${id}.label`),
    description: translateNow(`composer.promptTemplates.builtins.${id}.description`),
    text: translateNow(`composer.promptTemplates.builtins.${id}.text`)
  }))
}
